...
from src.models.resnet_bilateral.bc_inference import predict_w_resnet_bc
        ...
        for epoch in epoch_pbar:
            ...
            with tqdm(train_dl, leave=False, position=position, disable=not verbose) as pbar:
                for (left_batch, right_batch), output_batch in pbar:
                    ...
                    y_pred = model.patient_prediction(left_batch, right_batch) 
                    ...
                    left_batch, right_batch, loss = left_batch.detach().cpu(),\
                        right_batch.detach().cpu(), loss.detach().cpu()
                    del left_batch, right_batch, output_batch, y_pred, loss
            ...
            valid_outputs = predict_w_resnet_bc(model, valid_dl, verbose)
